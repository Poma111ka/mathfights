...
bot = telebot.TeleBot('5330645993:AAHu92oj-K2BysojQhIJE0GY4qTwgF5392o');
...
def get_text_messages(message):
    ...
    if str(message.text).lower() == "привет":
        bot.send_message(message.from_user.id, "Привет, это проект Mathfights - Многофункциональный сервис для изучения математики. \n Для информации введите /help.")
    elif str(message.text).lower() == "/help":
        bot.send_message(message.from_user.id, "проект Mathfights - Многофункциональный сервис для изучения математики \n Список команд: \n /s - вывести свой рейтинг \n /t - Получить задачу\n /a - ответ - дать ответ на задачу")
